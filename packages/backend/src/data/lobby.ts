...
function doesLobbyExistWithId(id: string) {
  const statement = database
    .prepare("SELECT EXISTS(SELECT 1 FROM lobbies WHERE id = ?)")
    .pluck();
...
export function createLobby(): string {
  ...
  const statement = database.prepare<LobbyDBO>(
    "INSERT INTO lobbies (id, display_name) VALUES (@id, @display_name)"
  );
...
export function getLobbyFromDatabaseById(id: string) {
  const statement = database.prepare<string>(
    "SELECT * FROM lobbies WHERE id = ? LIMIT 1"
  );
